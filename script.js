...
(function initContactForm() {
  ...
  form.addEventListener('submit', async e => {
    ...
    if (!termsAgree.checked) {
      termsAgree.closest('.checkbox-item').style.outline = '1px solid #c0392b';
      termsAgree.closest('.checkbox-item').scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }
...
(function initTermsModal() {
  ...
  const openBtn      = $('#openTerms');
  const closeBtn     = $('#closeTerms');
  const closeBtn2    = $('#closeTerms2');
  const overlay      = $('#termsModalOverlay');
  const agreeBtn     = $('#agreeTerms');
  const termsCheck   = $('#termsAgree');
  ...
  // Clicking "I Agree" checks the box and closes
  agreeBtn.addEventListener('click', () => {
    if (termsCheck) {
      termsCheck.checked = true;
      termsCheck.closest('.checkbox-item').style.outline = '';
      // Trigger checkbox visual update
...
(function initStagingSlider() {
  ...
  function setPosition(x) {
    ...
    handle.style.transform = 'translateX(-50%) translateZ(0)';
  }
  ...
  // Prevent page scroll when touching the slider on mobile
